...
def load_coco_captions(path: str) -> List[str]:
    ...
    with open(path, "r") as infile:
        annotations = json.load(
            infile
        )  # dictionary -> {image_path: List[caption1, caption2, ...]}
    punctuations = [
        "!",
        '"',
        "#",
    ...
    for image_path in annotations:
        temp_captions = annotations[
            image_path
        ]  # List: [caption1, caption2, ...], captions for the ith image
        for caption in temp_captions:  # caption
            caption = caption.strip()  # removing space at the end of the caption
            if (
                caption.isupper()
            ):  # processing the special caption in the COCO Caption, e.g., 'A BOY IS PLAYING BASEBALL.'
                caption = caption.lower()
            caption = (
                caption[0].upper() + caption[1:]
            )  # capitalizing the first letter in the caption
            if (
                caption[-1] not in punctuations
            ):  # adding a '.' at the end of the caption if there are no punctuations.
                caption += "."
...
def load_stopwords() -> List[str]:
    ...
    stopwords_and_punctuations = [
        stopword.lower() for stopword in stopwords_and_punctuations
    ]
...
def load_visual_genome_entities(path: str, all_entities: bool = True) -> List[str]:
    ...
    with open(path, "rb") as infile:
        all_objects_attributes_relationships = pickle.load(
            infile
        )  # dictionary {'relationships': dict, 'attributes': dict, 'objects': dict}
    entities = all_objects_attributes_relationships[
        "objects"
    ]  # dictionary {'gqa': set, 'vg': set, 'joint': set}, joint = gqa + vg
    ...
    if all_entities:
        ...
        entities = [
            entity.lower().strip() for entity in entities if len(entity.split()) == 1
        ]
    entities.sort()  # sort
...
def load_open_image_entities(path: str, all_entities: bool = True) -> List[str]:
    ...
    if all_entities:
        ...
        entities = [
            entity for entity in open_image_entities if len(entity.split()) == 1
        ]
    entities.sort()  # sort
...
def load_vinvl_vg_entities(path: str, all_entities: bool = True) -> List[str]:
    ...
    with open(path, "r") as infile:
        annotations = json.load(
            infile
        )  # dictionary = {'label_to_idx':dict,'idx_to_label':dict,'attribute_to_idx':dict,'idx_to_attribute':dict,'predicate_to_idx':dict,'idx_to_predicate':dict,'object_count':dict,'attribute_count':dict,'predicate_count':dict,}
    vinvl_entities = annotations[
        "object_count"
    ]  # dictionary = {str: int, str: int, ...}
...
def load_entities_text(
    name_of_entities: str, path_of_entities: str, all_entities: bool = True
) -> List[str]:
    """
    Args:
...
if __name__ == "__main__":
    ...
    entities = load_entities_text(
        entities_text[entities_idx], entities_path[entities_idx]
    )
    print("entities text: ", entities[:10], type(entities), len(entities))
    # using entities text with a single word
    entities_idx = 4
    entities = load_entities_text(
        entities_text[entities_idx], entities_path[entities_idx], all_entities=False
    )
    print("entities text: ", entities[:10], type(entities), len(entities))
