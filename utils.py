...
def entities_process(
    ...
    for i in range(len(detected_entities)):
        ...
        if (
            detected_entity in people_vocabs
        ):  # transforming all person concept (man, woman, kid, ...) to the same word 'person'
            detected_entity = "person"
        elif (
            len(detected_entity) > 1
            and detected_entity not in stopwords
            and detected_entity in objects_vocabs
        ):  # only processing entities in visual genome
            pass
...
def compose_discrete_prompts(
    ...
    if len(process_entities) == 0:  # without entities
        ...
        for (
            ...
            process_entities
        ):  # gpt2 in transformer encoder ' ' + word into one token by default
            discrete_prompt += " " + entity + ","  # ' person, dog, park,'
        discrete_prompt = discrete_prompt[:-1]  # ' person, dog, park'
        discrete_prompt = (
            prompt_head + discrete_prompt + prompt_tail
        )  # 'There are person, dog, park in image.'

    entities_tokens = torch.tensor(
        tokenizer.encode(discrete_prompt)
    )  # (discrete_prompt_length, )
...
def parse_entities(
    ...
    for idx in range(len(detected_entities)):
        # entities processing
        process_entities = entities_process(
            args, detected_entities[idx], stopwords, people_vocabs, objects_vocabs
        )
...
def padding_captions(
    ...
    discrete_tokens: List[
        torch.Tensor
    ] = None,  # len = batch_size, [(n_seq1, ), (n_seq2, ), ...]
    ...
    if discrete_tokens is None:  # capdec
        masks = torch.cat(
            (torch.ones(len(masks), args.continuous_prompt_length), masks), dim=-1
        )  # (batch_size, continuous_prompt_length + caption_seq)
        captions_tokens_for_loss = torch.cat(
            (
                torch.zeros(
                    (len(captions_tokens), args.continuous_prompt_length),
                    dtype=torch.int64,
                ),
                captions_tokens,
            ),
        ...
        captions_tokens_for_loss = torch.cat(
            (
                captions_tokens_for_loss[:, 1:],
                torch.zeros((len(captions_tokens), 1), dtype=torch.int64),
            ),
            dim=-1,
        )
        ...
        hard_prompts_length = []
        max_length = (
            2 * args.max_num_of_entities
            - 1
            + args.prompt_template_length
            + captions_tokens.shape[-1]
        )  # max length without soft prompt
        for i in range(len(discrete_tokens)):
            tokens = torch.cat((discrete_tokens[i], captions_tokens[i]))
            loss_tokens = torch.cat(
                (
                    torch.zeros((len(discrete_tokens[i])), dtype=torch.int64),
                    captions_tokens[i],
                )
            )
            ...
            if padding > 0:
                ...
                loss_tokens = torch.cat(
                    (loss_tokens, torch.zeros((padding), dtype=torch.int64))
                )
            ...
            if args.only_hard_prompt:
                ...
                loss_tokens = torch.cat(
                    (
                        torch.zeros((args.continuous_prompt_length), dtype=torch.int64),
                        loss_tokens[:max_length],
                    )
                )
            loss_tokens = torch.cat(
                (loss_tokens[1:], torch.zeros((1), dtype=torch.int64))
            ).unsqueeze(
                dim=0
            )  # (1, max_length + continuous_prompt_length)
            if captions_tokens_with_hard_prompts is None:
                captions_tokens_with_hard_prompts = tokens
                captions_tokens_for_loss = loss_tokens
            else:
                captions_tokens_with_hard_prompts = torch.cat(
                    (captions_tokens_with_hard_prompts, tokens), dim=0
                )
                captions_tokens_for_loss = torch.cat(
                    (captions_tokens_for_loss, loss_tokens), dim=0
                )
            ...
            if padding > 0:
                if args.only_hard_prompt:
                    temp_masks = torch.cat(
                        (
                            torch.ones(hard_prompts_length[-1]).float(),
                            masks[i],
                            torch.zeros(padding).float(),
                        )
                    )
                else:
                    temp_masks = torch.cat(
                        (
                            torch.ones(
                                hard_prompts_length[-1] + args.continuous_prompt_length
                            ).float(),
                            masks[i],
                            torch.zeros(padding).float(),
                        )
                ...
                if args.only_hard_prompt:
                    temp_masks = torch.cat(
                        (torch.ones(hard_prompts_length[-1]).float(), masks[i])
                    )
                else:
                    temp_masks = torch.cat(
                        (
            ...
            if args.only_hard_prompt:
                ...
                temp_masks = temp_masks[
                    : max_length + args.continuous_prompt_length
                ].unsqueeze(dim=0)
        ...
        return (
            captions_tokens_with_hard_prompts,
            captions_tokens_for_loss,
            padding_masks,
            hard_prompts_length,
        )
