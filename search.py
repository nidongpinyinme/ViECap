...
def opt_search(
    ...
    if embeddings is not None:
        ...
        if tokens is None:
            ...
            )  # (n_seq), tokenizing prompts
            tokens = tokens.unsqueeze(dim=0).to(
                device
            )  # (b(=1), n_seq), adding batch dimension
    ...
    prompt = [prompt] * b
    opt_tokens = tokenizer(prompt, add_special_tokens=False, return_tensors="pt").to(
        embeddings.device
    )
...
def greedy_search(
    ...
    # tokenizing end of sentence, when the length of eos tokens is greater than 1, setting the first token of eos tokens as eos token
    eos = [
        tokenizer.encode(end_of_sentence)[-1] for end_of_sentence in end_of_sentences
    ]
    ...
    for step in range(max_len):
        ...
        if step == 0:
            outputs = model(
                inputs_embeds=generating.type(model.dtype),
                past_key_values=past_key_values,
                use_cache=True,
            )
            next_token_logits = outputs.logits[
                :, -1, :
            ]  # (b, n_seq, vocal_size) -> (b, vocal_size), logits of the last token
            past_key_values = (
                outputs.past_key_values
            )  # Tuple[Tuple[(b, h, n_seq, lm_hidden_size/h)]], layers -> (key, value) -> torch.tensor
        ...
        outputs = model(
            inputs_embeds=next_embedding.type(model.dtype),
            past_key_values=past_key_values,
            use_cache=True,
        )
        next_token_logits = outputs.logits[
            :, -1, :
        ]  # (b, 1, vocal_size) -> (b, vocal_size)
        past_key_values = (
            outputs.past_key_values
    ...
    for i in range(max_len):
        ...
        if scores is None:
            ...
            next_tokens_source = torch.div(
                next_tokens, scores_sum.shape[1], rounding_mode="trunc"
            )
        ...
        next_token_embed = word_embed(model, next_tokens.squeeze()).view(
        ...
        is_stopped = (
            is_stopped + (next_tokens.eq(eos[0]) | next_tokens.eq(eos[1])).squeeze()
        )
        if is_stopped.all():
            break
...
def contrastive_search(
    ...
    for step in range(max_len):
        ...
        if step == 0:
            ...
            past_hidden_states = outputs.hidden_states[
                -1
            ]  # Tuple[(b, n_seq, lm_hidden_size)] -> (b, n_seq, lm_hidden_size) (i.e., hidden state of last layer)
        ...
        _, top_k_indices = torch.topk(
            next_token_logits, dim=-1, k=top_k
        )  # (b, k), the indices for top k candidates (i.e., tokens)
        top_k_probs = torch.gather(
            next_token_probs, dim=1, index=top_k_indices
        )  # (b, k), the probability for top k candidates

        # transformering b*k tokens to embeddings and processing past_key_values to compute simultaneously for k tokens
        top_k_embeddings = model.transformer.wte(
            top_k_indices.view(-1, 1)
        )  # (b*k, 1, lm_hidden_size)
        past_key_values = reshape_from_past_key_values(
            past_key_values, top_k
        )  # Tuple[Tuple[(b*k, h, n_seq, lm_hidden_size/h)]]
        # computing hidden state of next token (b * top_k in total)
        outputs = model(
            inputs_embeds=top_k_embeddings,
            past_key_values=past_key_values,
            use_cache=True,
            output_hidden_states=True,
        )
        ...
        past_key_values = (
            outputs.past_key_values
        )  # Tuple[Tuple[(b*k, h, n_seq + 1, lm_hidden_size/h)]]
        next_hidden_state = outputs.hidden_states[
            -1
        ]  # Tuple[(b*k, 1, lm_hidden_size)] -> (b*k, 1, lm_hidden_size)
        context_hidden_states = (
            past_hidden_states.unsqueeze(dim=1)
            .expand(-1, top_k, -1, -1)
            .reshape(b * top_k, n_seq, lm_hidden_size)
        )  # (b*k, n_seq, lm_hidden_size)

        # selecting next token within top k candidates for each sentence
        selected_max_prob_indices = ranking_and_selecting(
            context_hidden_states, next_hidden_state, top_k_probs, alpha, top_k
        )  # (b)
        ...
        next_token_logits = logits[
            range(b), selected_max_prob_indices, :
        ]  # (b, vocal_size)
        past_key_values = reshape_to_past_key_values(
            past_key_values, selected_max_prob_indices, top_k
        )  # (b, h, n_seq + 1, lm_hidden_size/h)
        next_hidden_state = torch.stack(
            torch.split(next_hidden_state.squeeze(dim=1), top_k), dim=0
        )  # (b, k, lm_hidden_size)
        next_hidden_state = next_hidden_state[
            range(b), selected_max_prob_indices, :
        ]  # (b, lm_hidden_size)
        past_hidden_states = torch.cat(
            [past_hidden_states, next_hidden_state.unsqueeze(dim=1)], dim=1
        )  # [b, n_seq + 1, lm_hidden_size]

        # computing next token and saving it
        next_token = top_k_indices[range(b), selected_max_prob_indices].unsqueeze(
            dim=-1
        )  # (b, 1)
        if tokens is None:
            tokens = next_token
...
def magic_search(
    ...
    if embeddings is not None:
        ...
        generating = model.transformer.wte(
            tokens
        )  # (b, n_seq, lm_hidden_size), transforming to word embeddings
    generating = generating.float()  # (b, n_seq, lm_hidden_size)
    assert generating.dim() == 3, "The dimension of prompts should equal to 3!"
    ...
    if images_feature is None:
        image = (
            preprocess(Image.open(image_path)).unsqueeze(dim=0).to(device)
        )  # (b(=1), 3, 224, 224)
        images_feature = clip_model.encode_image(image)  # (b, clip_hidden_size)
    images_feature = images_feature.float()  # (b, clip_hidden_size)
    assert (
        images_feature.dim() == 2
    ), "The dimension of images feature should equal to 2!"
    assert (
        images_feature.shape[0] == generating.shape[0]
    ), "The number of images should be equal to the number of prompts/tokens/embeddings!"
    ...
    for step in range(max_len):
        ...
        image_sentence_score = image_sentence_similarity(
            tokens_generated,
            top_k_indices,
            images_feature,
            top_k,
...
def image_sentence_similarity(
    ...
    if tokens_generated is None:
        temp_tokens = top_k_indices.view(-1).unsqueeze(
            dim=1
        )  # (b*k, n_seq + 1), where n_seq = 0
    else:
        b, n = tokens_generated.size()
        tokens_generated = (
            tokens_generated.unsqueeze(dim=1)
            .expand(-1, top_k, -1)
            .reshape(b * top_k, n)
        )  # (b*k, n_seq)
        top_k_indices = top_k_indices.view(-1).unsqueeze(dim=1)  # (b*k, 1)
        temp_tokens = torch.cat(
            [tokens_generated, top_k_indices], dim=1
        )  # (b*k, n_seq + 1)
    ...
    clip_embeddings = torch.stack(
        torch.split(clip_embeddings, top_k), dim=0
    ).float()  # (b, k, clip_hidden_size)
    ...
    clip_embeddings = clip_embeddings / clip_embeddings.norm(
        dim=-1, keepdim=True
    )  # (b, k, clip_hidden_size)
    images_feature = images_feature / images_feature.norm(
        dim=-1, keepdim=True
    )  # (b, 1, clip_hidden_size)
    scaling = clip_model.logit_scale.exp()
    score = (
        torch.matmul(clip_embeddings, images_feature.transpose(1, 2)).squeeze(dim=2)
        * scaling
    )  # (b, k)
...
def reshape_from_past_key_values(
    past_key_values: Tuple[Tuple[torch.Tensor]], top_k: int
    ...
    for layer in past_key_values:
        ...
        for item in layer:
            ...
            item = (
                item.unsqueeze(dim=1)
                .expand(-1, top_k, -1, -1, -1)
                .reshape(b * top_k, h, n, d)
            )  # (b*k, h, n_seq, lm_hidden_size/h)
            items.append(item)
        new_key_values.append(items)
    return new_key_values


def reshape_to_past_key_values(
    ...
    for layer in past_key_values:
        ...
        for item in layer:
            ...
            )  # (b, k, h, n_seq + 1, lm_hidden_size/h)
            item = item[
                range(b), selected_max_prob_indices, :, :, :
            ]  # (b, h, n_seq + 1, lm_hidden_size/h)
            items.append(item)
...
def ranking_and_selecting(
    ...
    norm_context_hidden_states = context_hidden_states / context_hidden_states.norm(
        dim=-1, keepdim=True
    )  # (b*k, n_seq, lm_hidden_size)
    norm_next_hidden_state = next_hidden_state / next_hidden_state.norm(
        dim=-1, keepdim=True
    )  # (b*k, 1, lm_hidden_size)
    ...
    cosine_matrix = torch.matmul(
        norm_context_hidden_states, norm_next_hidden_state.transpose(1, 2)
    ).squeeze(
        -1
    )  # (b*k, n_seq)
    ...
    if image_sentence_score is not None:
        scores = (
            (1.0 - alpha) * top_k_probs - alpha * scores + beta * image_sentence_score
        )  # (b*k)
    else:
