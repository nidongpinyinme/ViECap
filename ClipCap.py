...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        )  # (batch_size, n_seq, num_heads, head_size)
        keys_values = self.to_keys_values(key_value).reshape(
            b, m, 2, self.num_heads, self.head_size
        )  # (batch_size, m_seq, 2, num_heads, head_size)
        keys, values = (
            keys_values[:, :, 0],
            keys_values[:, :, 1],
        )  # (batch_size, m_seq, num_heads, head_size), (batch_size, m_seq, num_heads, head_size)
        attention = (
            torch.einsum("bnhd,bmhd->bnmh", queries, keys) * self.scale
        )  # (batch_size, n_seq, m_seq, num_heads)

        if mask is not None:
            if mask.dim() == 2:
                mask = mask.unsqueeze(
                    dim=1
                )  # expending dimension, shape: (batch_size, 1, m_seq)
            attention = attention.masked_fill(
                mask.unsqueeze(dim=3), float("-inf")
            )  # expending dimension n_seq head and fill -inf according to mask

        attention = attention.softmax(
            dim=2
        )  # softmax alongside the dimension of key_value pairs
        outputs = torch.einsum("bnmh,bmhd->bnhd", attention, values).reshape(
            b, n, d_query
        )  # (batch_size, n_seq, d_query)
...
class TransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.attn = MultiHeadAttention(
            query_size, key_value_size, num_heads, bias=bias, dropout=dropout
        )
        self.norm2 = norm_layer(query_size)
        self.mlp = MlpTransformer(
            query_size, int(query_size * mlp_ratio), act=act, dropout=dropout
        )
...
class MappingNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        )  # (b, clip_project_length, d_model)
        prefix = self.prefix_const.unsqueeze(dim=0).expand(
...
class ClipCaptionModel(nn.Module):
    ...
    def __init__(
        ...
        self.prior = get_prior_model(
            "../../../checkpoints/DALLE/prior_config.json",
            "../../../checkpoints/DALLE/prior.pth",
        )
    ...
    def forward(
        ...
        caption_gpt_tokens: torch.Tensor,
        ...
        Args:
            ...
            caption_gpt_tokens: caption tokens with a shape of (b, max_length_per_caption)
