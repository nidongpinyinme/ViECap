...
def clip_texts_embeddings(
    ...
    epochs = (
        int(num_categories / batch_size)
        if num_categories % batch_size == 0
        else 1 + int(num_categories // batch_size)
    )
    for epoch in range(epochs):
        temp_texts_tokens = clip_texts_tokens[
            batch_size * epoch : batch_size * (epoch + 1)
        ]  # (batch_size/(num_categories % batch_size), 77)
        temp_texts_tokens = temp_texts_tokens.to(device)
        with torch.no_grad():
            temp_texts_embeddings = (
                model.encode_text(temp_texts_tokens).float().to("cpu")
            )  # (batch_size/(num_categories % batch_size), clip_hidden_size)
        if texts_embeddings is None:
            texts_embeddings = temp_texts_embeddings
        else:
            texts_embeddings = torch.cat(
                (texts_embeddings, temp_texts_embeddings), dim=0
            )
...
def image_text_simiarlity(
    ...
    if images_features is None:
        ...
        ), "Either image path or images feature should be given!"
        image = (
    ...
    images_features = images_features.float().to(
        "cpu"
    )  # (num_images, clip_hidden_size)
    texts_embeddings = texts_embeddings.float().to(
        "cpu"
    )  # (num_categories, clip_hidden_size)
    images_features /= images_features.norm(
        dim=-1, keepdim=True
    )  # (num_images, clip_hidden_size)
    texts_embeddings /= texts_embeddings.norm(
        dim=-1, keepdim=True
    )  # (num_categories, clip_hidden_size)

    image_to_text_similarity = (
        torch.matmul(images_features, texts_embeddings.transpose(1, 0)) / temperature
    )  # (num_imegs, num_categories)
    image_to_text_logits = torch.nn.functional.softmax(
        image_to_text_similarity, dim=-1
    )  # (num_imegs, num_categories)
...
def top_k_categories(
    ...
    threshold: Optional[
        float
    ] = 0.0,  # probability which is less than threshold will be filtered
) -> Tuple:

    top_k_probs, top_k_indices = torch.topk(
        logits, k=top_k, dim=-1
    )  # (num_images, top_k)
    top_k_texts = []
